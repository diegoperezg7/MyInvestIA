...
async def get_watchlists(user: AuthUser = Depends(get_current_user)):
    ...
    raw = store.get_watchlists(user.id)
...
async def get_watchlist(watchlist_id: str, user: AuthUser = Depends(get_current_user)):
    ...
    raw = store.get_watchlist(user.id, watchlist_id)
    if not raw:
...
async def create_watchlist(req: CreateWatchlistRequest, user: AuthUser = Depends(get_current_user)):
    """Create a new watchlist."""
    raw = store.create_watchlist(user_id=user.id, name=req.name)
    return Watchlist(**raw)


@router.patch("/{watchlist_id}", response_model=Watchlist)
async def update_watchlist(watchlist_id: str, req: UpdateWatchlistRequest, user: AuthUser = Depends(get_current_user)):
    """Rename a watchlist."""
    raw = store.update_watchlist(user.id, watchlist_id, name=req.name)
    if not raw:
        raise HTTPException(status_code=404, detail="Watchlist not found")
    return Watchlist(**raw)
...
async def delete_watchlist(watchlist_id: str, user: AuthUser = Depends(get_current_user)):
    """Delete a watchlist."""
    if not store.delete_watchlist(user.id, watchlist_id):
        raise HTTPException(status_code=404, detail="Watchlist not found")


@router.post("/{watchlist_id}/assets", response_model=Watchlist)
async def add_asset_to_watchlist(watchlist_id: str, req: AddWatchlistAssetRequest, user: AuthUser = Depends(get_current_user)):
...
async def remove_asset_from_watchlist(watchlist_id: str, symbol: str, user: AuthUser = Depends(get_current_user)):
    """Remove an asset from a watchlist."""
    raw = store.remove_asset_from_watchlist(user.id, watchlist_id, symbol)
    if not raw:
        raise HTTPException(status_code=404, detail="Watchlist not found")
    return Watchlist(**raw)
