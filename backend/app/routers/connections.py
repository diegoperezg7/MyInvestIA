...
async def create_exchange_connection(req: CreateExchangeConnectionRequest):
...
async def create_wallet_connection(req: CreateWalletConnectionRequest):
...
async def create_broker_connection(req: CreateBrokerConnectionRequest):
...
async def create_prediction_connection(req: CreatePredictionConnectionRequest):
...
async def create_polymarket_connection(req: CreatePredictionConnectionRequest):
    """Backwards-compatible endpoint — redirects to /prediction."""
...
async def delete_connection(connection_id: str):
    ...
    if not connection_service.delete_connection(connection_id):
        raise HTTPException(status_code=404, detail="Connection not found")
...
async def sync_connection(connection_id: str):
    ...
    try:
        return await connection_service.sync_connection(connection_id)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def sync_all():
    ...
    return await connection_service.sync_all()
...
async def test_connection(connection_id: str):
    ...
    try:
        return await connection_service.test_connection_by_id(connection_id)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))


@router.get("/{connection_id}/history")
async def get_sync_history(connection_id: str, limit: int = Query(default=20, le=100)):
    """Get synchronization history for a connection."""
    conn = connection_service.get_connection(connection_id)
    if not conn:
        raise HTTPException(status_code=404, detail="Connection not found")
    return connection_service.get_sync_history(connection_id, limit)
